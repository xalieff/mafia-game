 ...
 *   start_game     {}                           (host, lobby only, >= 4 players)
 ...
 *   game_started           { playerCount, mafiaCount }
 ...
 * @property {{mafiaVotes: Map<string,string>, doctorSave: string|null, detectiveTarget: string|null}} night
...
const freshNight = () => ({ mafiaVotes: new Map(), doctorSave: null, detectiveTarget: null });
...
function nightChoiceOf(room, player) {
  switch (player.role) {
    ...
    case ROLE.DETECTIVE:
      return room.night.detectiveTarget;
    default:
      return null;
  }
}
...
function stateFor(room, viewer) {
  ...
  if (room.phase === PHASE.NIGHT && viewer.alive && NIGHT_ROLES.has(viewer.role)) {
    state.night = {
      ...
      locked: viewer.role === ROLE.DETECTIVE && room.night.detectiveTarget !== null,
      teamPicks:
        viewer.role === ROLE.MAFIA
...
function startGame(room) {
  const connected = allPlayers(room).filter((p) => p.connected);
  if (connected.length < CFG.minPlayers) {
    throw new GameError(`At least ${CFG.minPlayers} connected players are required to start.`);
  }
  ...
  const players = secureShuffle(allPlayers(room));
  const n = players.length;
  const mafiaCount = Math.max(1, Math.floor(n / 4)); // 4-7 → 1, 8-11 → 2, 12-15 → 3
  const roles = [...Array(mafiaCount).fill(ROLE.MAFIA), ROLE.DOCTOR, ROLE.DETECTIVE];
  while (roles.length < n) roles.push(ROLE.TOWN);
  ...
  emitToRoom(room, 'game_started', { playerCount: n, mafiaCount });
  beginNight(room);
}
...
function hasActed(room, player) {
  switch (player.role) {
    ...
    case ROLE.DOCTOR:
      return room.night.doctorSave !== null;
    case ROLE.DETECTIVE:
      return room.night.detectiveTarget !== null;
    default:
      return true;
  }
}
...
function resolveNight(room) {
  ...
  const saved = room.night.doctorSave;
  const victim = targetId && targetId !== saved ? room.players.get(targetId) : null;
...
io.on('connection', (socket) => {
  ...
  handle(socket, 'night_action', (data) => {
    ...
    switch (player.role) {
      ...
      case ROLE.DOCTOR:
        room.night.doctorSave = target.id; // self-save allowed; can change until night ends
        break;

      case ROLE.DETECTIVE: {
        if (room.night.detectiveTarget !== null) {
        ...
        room.night.detectiveTarget = target.id;
